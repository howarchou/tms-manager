...
const Step3: React.FC<Step3Props> = ( props ) => {
  ...
  const [ cacheButton, allSubmitButton ] = useBuild(data ?? {} as API.TeamBuildingNew, form, dispatch, submitting)
  useEffect(() => {
    form.setFieldsValue({
      ...data,
      schedule: data?.schedule ?? [ { items: [ {} ] } ]
    })
  }, [ data, form ])
  if (!data) {
    return null
  ...
  return (
    <Form style={ { height: '100%', marginTop: 40 } }
      form={ form }
      layout='vertical'
      autoComplete='off'
      initialValues={ data }>
                  ...
                  <Card type='inner'
                    title={
                      <Row key={ field.key }
                        ...
                        <Col span={ FormItemTitleLayoutSpan }
                              ...
                              { fields.map(( field, index ) => (
                                <div key={ field.key }>
                                  ...
                                  <Row gutter={ FormRowLayoutSpan }
                                    ...
                                    <Form.Item className={ styles.actionRowWrapper }
                                      { ...field }
                                      name={ [ field.name, 'pictures' ] }
                                      fieldKey={ [ field.fieldKey, 'pictures' ] }>
                                      <UploadComponent showUploadList={ true }
                                        multiple={ true }
                                        label={ '上传图片' }
                                        max={ 100 }/>
                                    </Form.Item>
