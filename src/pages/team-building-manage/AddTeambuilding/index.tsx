...
const AddTeamBuilding: React.FC<AddTeamBuildingProps> = ( {
                                                            current,
                                                            location
                                                          } ) => {
  ...
  return (
    <PageContainer title={ <HeaderBack title={ id && !isCopy ? '编辑团建' : '添加团建' }
      onBackClick={ handleLeftClick }/> }>
      <Card bordered={ false }>
...
const AddTeamBuildingWrapper = ( props: any ) => {
  ...
  useEffect(() => {
    if (id) {
      ...
      if (!isCopy) {
        const buildInfoStr = localStorage.getItem('tms_build_info')
        if (buildInfoStr) {
          Modal.confirm({
            title: '确定从草稿加载数据？',
            icon: <ExclamationCircleOutlined/>,
            content: '如果取消，将清空本地草稿，下次不会再弹出本确认框。',
            onOk() {
              const buildInfo = JSON.parse(buildInfoStr)
              handleUpdateData(buildInfo)
            },
            onCancel() {
              localStorage.removeItem('tms_build_info')
            },
          })
          return
        }
