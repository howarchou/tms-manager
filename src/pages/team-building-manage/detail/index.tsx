...
export default function (props: Props) {
  ...
  const LeftTitle = () => {
    return (
      <>
        <ArrowLeftOutlined onClick={handleLeftClick} style={{marginRight: 8}}/> 团建详情 </>
    )
  }

  return (
    <PageContainer title={<LeftTitle/>}> <Descriptions className="detail_wrapper" title={'基本信息'}>
...
function RateCP(props: ComponentProps) {
  ...
  return (
    <>
      <Rate className="value" value={Number(1)} disabled={true}/> <Rate className="value" value={Number(2)} disabled={true}/> <Rate className="value" value={Number(3)}
                                                                                                                                    disabled={true}/> <Rate className="value"
                                                                                                                                                            value={Number(4)}
                                                                                                                                                            disabled={true}/> <Rate
      className="value" value={Number(5)} disabled={true}/>
    </>
  )
}
